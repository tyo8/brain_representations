...
def main(argvals):
    ...
    measure_phom_dists(phom_dgms_fname,fname_out=phom_dist_fname)
...
def measure_phom_dists(fname_in='', phom_dgms=np.asarray([]), 
...
def comp_Betti_curves(fname_in='', phom_dgms=np.asarray([]), 
        subdir="figures", fname_out="Betti_curves.npy",
    ...
    if plot:
        fig_dir = os.path.join(os.path.dirname(fname_out), subdir)
