...
def permtree(permutation_tree, perms, conditional_monte_carlo=False, max_perms=None, verbose=False):
    permutation_tree = deepcopy(permutation_tree)
    permutation_set = pickperm(permutation_tree, [])
    permutation_set = np.hstack(
        (
            np.array(permutation_set, ndmin=2).T,
            np.zeros((len(permutation_set), perms - 1)),
        )
    )
    ...
    if verbose:
        print(f"\nRequested {perms} permutations of a possible {max_perms}.\n")
    ...
    if perms == 1:
        ...
        if perms > 1e5:
            warnings.warn(
                f"Number of possible sign flips is {max_perms}. Performing all exhaustively"
            )
        ...
        if verbose:
            print("Computing exhaustive permutations (either \'conditional_monte_carlo\'=True or perms > max_perms)")
            print(f"Conditional Monte Carlo: {conditional_monte_carlo}")
            print(f"perms > max_perms = {perms > max_perms}")
        for p in range(1, min(perms,max_perms)):
            permutation_tree = randomperm(permutation_tree)
            permutation_set[:, p] = pickperm(permutation_tree, [])
    else:
        if perms > max_perms / 2:
            warnings.warn(
                f"The maximum number of sign flips {max_perms} is not much larger than"
        ...
        for p in range(1, perms):
            whiletest = True
            # print(f"doing permutation number {p}...")
            while whiletest:
                permutation_tree = randomperm(permutation_tree)
                permutation_set[:, p] = pickperm(permutation_tree, [])
                whiletest = np.any(
        ...
        for u in range(nU):
            ...
            if sucs[u]:
                ...
                if sucs[u]:
                    ...
                    permutation_tree[u][2] = permutation_tree[u][2][
                        permutation_tree[u][0][:, 2]
                    ]
                    if u > 0:
                        permutation_tree[: u - 1, :] = resetperms(
    ...
    if len(permutation_tree[0]) > 1:
        for u in range(len(permutation_tree)):
            if np.isnan(permutation_tree[u][0]):
                ...
                permutation_tree[u][0], idx = permutation_tree[u][0][
                    np.argsort(permutation_tree[u][0][:, 0])
                ]
    ...
    for u in range(nU):
        if not np.any(np.isnan(permutation_tree[u][0])):
            ...
            if np.any(tmp != permutation_tree[u][0][0]):
                permutation_tree[u][2] = [
                    permutation_tree[u][2][k] for k in permutation_tree[u][0][2]
                ]
