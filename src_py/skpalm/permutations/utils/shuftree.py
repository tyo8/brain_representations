...
def shuftree(
    ...
    if verbose:
        print(f"Number of permuations requested: {perms}\n")
    ...
    if exchangeable_errors:
        ...
        if np.isinf(maxP):
            if verbose:
                print(f"Number of possible permutations is exp({lmaxP}). Setting to artificial cap: {max_cap}\n")
            maxP = max_cap
        else:
            if verbose:
    ...
    if is_errors:
        ...
        if np.isinf(maxS):
            if verbose:
                print(f"Number of possible permutations is exp({lmaxS}). Setting to artificial cap: {max_cap}\n")
            maxS = max_cap
        else:
            if verbose:
    ...
    if perms == 0 or perms > maxB:
        ...
        if exchangeable_errors:
            permutation_set = permtree(
                permutation_tree, int(maxP), int(maxP), verbose=verbose
            )
        if is_errors:
            Sset = fliptree(permutation_tree, int(maxP), int(maxP))
    elif perms < maxB:
        if exchangeable_errors:
            if perms > maxP:
                permutation_set = permtree(
                ...
                permutation_set = permtree(
                    permutation_tree, perms, conditional_monte_carlo, int(maxP)
                )
        if is_errors:
            if perms > maxS:
                Sset = fliptree(
                ...
                Sset = fliptree(
                    permutation_tree, perms, conditional_monte_carlo, int(maxS)
                )
