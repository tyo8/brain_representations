...
def one_displot(
    ...
    if write_mode:
        outname = "displot.png"
        if log_scale:
            outname = outname.replace("displot","displot-log")
        if extrema_only:
            outname = outname.replace("displot","displot-extremal")
        for var in ['x', 'y', 'hue', 'row', 'col']:
            varname = eval(f"{var}_var")
            if varname is not None:
                outname = outname.replace("displot", f"displot_{var}-{varname}")
...
if __name__=="__main__":
    ...
    if args.fpathlist_path is None: 
        search_pattern = os.path.join(args.input_dir, dir_pattern, f_pattern)
        fpath_list = glob.glob(search_pattern)
    else:
        with open(args.fpathlist_path, 'r') as fin:
            fpath_list = fin.read().split('\n')

    full_df = pd.concat([ _load(fpath, extrema_only=args.extrema_only) for fpath in fpath_list ], ignore_index=True)

    if args.verbose:
        print(f"collected summary dataframe: \n{full_df}")
        for colname in full_df.columns.values:
            print(f"Number of unique values in column \'{colname}\' is {len(set(full_df[colname].values))}")
    ...
    if not args.extrema_only:
        for dist_var in dist_vars:
            for var in ["modality", "feature", "metric"]:
                full_df[var] = full_df.apply(lambda x: frozenset([x[f"X_{var}"], x[f"Y_{var}"]]), axis=1)
                row_var = f"X_{var}"
                col_var = f"Y_{var}"
                one_displot(
                        full_df,
