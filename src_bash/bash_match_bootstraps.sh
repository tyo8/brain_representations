...
do
		...
		sbatch_fname=$(dirname ${distname})"/do_match_${data_label}"
		...
		phomX_fname="${outdir}/phom_X.txt"
		# submit persistence job for dX
		###############
		python ${xtr_script} -x ${phomX_fname} -0 -w 
		###############

    		for tag in $tags
		do
			# name the (generic type of) output file for persistent homology data
			phomY_fname=${phomdir}"/phomY_${tag}.txt"
			if ! [ -f ${phomY_fname} ]
			then
			...
			python ${xtr_script} -x ${phomY_fname} -0 -w
			python ${xtr_script} -x ${phomY_fname/phomY/phomXZ} -0 -w -i
			python ${xtr_script} -x ${phomY_fname/phomY/phomYZ} -0 -w -i 
		
			# script submitter for matching job
			${match_scripter} -x ${phomX_fname} -y ${phomY_fname} -D ${match_homdim} -f ${sbatch_fname} -d ${data_label} -m ${mem_gb} -s ${subbase_dir}
		done
	done
done
